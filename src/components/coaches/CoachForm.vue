<template>
    <form action="" @submit.prevent="submitFrom">
        <div class="form-control" :class="{ invalid: !firstName.isValid }">
            ...
            <input type="text" name="" id="firstname" v-model.trim="firstName.val" @blur="clearValidity('firstName')">
            <p v-if="!firstName.isValid">Fistname must not be empty.</p>
        </div>
        <div class="form-control" :class="{ invalid: !lastName.isValid }">
            <label for="secondname">Secondname</label>
            <input type="text" name="" id="secondname" v-model.trim="lastName.val" @blur="clearValidity('lastName')">
            <p v-if="!lastName.isValid">Lastname must not be empty.</p>
        </div>
        <div class="form-control" :class="{ invalid: !description.isValid }">
            <label for="description">Description</label>
            <textarea type="text" name="" id="description" rows="5" v-model.trim="description.val"
                @blur="clearValidity('description')"></textarea>
            <p v-if="!description.isValid">Description must not be empty.</p>
        </div>
        <div class="form-control" :class="{ invalid: !rate.isValid }">
            <label for="rate">Hourly Rate</label>
            <input type="number" name="" id="rate" v-model.trim="rate.val" @blur="clearValidity('rate')">
